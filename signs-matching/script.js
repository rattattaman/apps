...
function loadWins() {
  return parseInt(localStorage.getItem('sm_wins') || '0', 10);
}
function saveWins(n) {
  localStorage.setItem('sm_wins', String(n));
}
function loadSeen() {
  try { return new Set(JSON.parse(localStorage.getItem('sm_seen') || '[]')); } catch { return new Set(); }
}
function saveSeen(set) {
  localStorage.setItem('sm_seen', JSON.stringify(Array.from(set)));
}
function loadMedals() {
  try {
...
function saveMedals(obj) {
  localStorage.setItem('sm_medals', JSON.stringify(obj));
}
...
function onWin(deck) {
  ...
  if (medals.gold?.at === now) showToast('Â¡Medalla de oro!', 'assets/medals/gold.svg');
  else if (medals.silver?.at === now) showToast('Â¡Medalla de plata!', 'assets/medals/silver.svg');
  else if (medals.bronze?.at === now) showToast('Â¡Medalla de bronce!', 'assets/medals/bronze.svg');
  if (medals.explorer?.at === now) showToast('Â¡Medalla de explorador!', 'assets/medals/explorer.svg');
}
